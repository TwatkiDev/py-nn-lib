...
class SimpleMatrix(MatrixInterface):
    ...
    @staticmethod
    def to_array(m):
        if not isinstance(m, SimpleMatrix):
    ...
    def copy(m):
        if not isinstance(m, SimpleMatrix):
            print("is not a matrix")
            return None
        _m = SimpleMatrix(m.rows, m.columns)
    ...
    def matrix_product(a, b):
        ...
        ret = SimpleMatrix(a.rows, b.columns)
    ...
    @staticmethod
    def transpose(m):
        ret = SimpleMatrix(m.columns, m.rows)
        for idx in range(len(m.matrix)):
            r = int(idx / m.columns)
            c = idx % m.columns
    ...
    def __eq__(self, o):
        if not isinstance(o, SimpleMatrix):
            return False
